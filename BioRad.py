...
SampleTabLine1 = (
    "CDM Export A1c Laboratories Instrument #1 07/23/2021 Run #25 V2TURBO_A1c\n"
)


SampleTabLine3 = (
    "Run #	Inj #	RackID	Type	Sample ID / Lot Number	Injection Date	Injection Time	Peak Name	RT	"
    "Concentration % NGSP\n"
)
...
def dataSimulation():
    ...
    for list in dateList:
        ...
        for key in clientDictionary:
            cur.execute(
                "INSERT INTO MasterOrder (ClientID, ClientCode, SampleStartNum, SampleEndNum) VALUES(?, ?, ?, ?)",
                (key, clientDictionary[key], sampleStartNum, sampleEndNum),
            )
            conn.commit()
        ...
        sampleResults.extend(
            range(dataS, (random_number + dataS), 1)
        )  # First Param is optional and starts at zero.
        # Range is immutable so need to shuffle so assigned range to list
        ...
        random_a1c = 0.0
        random_a1c = round(
            random.uniform(4.2, 10.5), 1
        )  # Gets number to one decimal precision using round params
                ...
                for cell in row:
                    ...
                    if sheet[CellRef].value:
                        ...
                        if sheet[cell.coordinate].value is None:
                            ...
                            if cell.column == 7:
                                print("format dates")
                                datev = datetime.strftime(
                                    sheet[cell.coordinate].value, "%m/%d/%Y"
                                )
                                print(datev)
        ...
        try:
            ...
            with open(csvfilename, newline="") as a1cxl:
                ...
                for line in csvfile:
                    ...
                    # https://stackoverflow.com/questions/54261748/skip-first-row-in-openpxyl/54262175
                    # "=SUM(1, 1)"
                    sheet.cell(row=rowNum, column=5).value = (
                        "=CONCATENATE(ROUND((28.7 * C"
                        + str(rowNum)
                        + ')-46.7,0), " mg/dl")'
                    )
                    sheet.cell(row=rowNum, column=10).value = (
                        "=(H" + str(rowNum) + "* 12) + I" + str(rowNum)
                    )
            ...
            messagebox.showinfo(
                "showinfo", "No Excel File Created \nFirst Read BioRad File."
            )
                ...
                if count > 3:
                    if result[3] == "P":
                        if result[7] == "A1c":
                            ...
                            cur.execute(
                                "SELECT SampleID FROM MachineResults WHERE SampleID = ?",
                                (result[4],),
                            )
                            data = cur.fetchall()
                            if len(data) == 0:
                                print("Insert Record")
                                cur.execute(
                                    "INSERT INTO MachineResults (SampleID, Date, A1c) VALUES(?, ?, ?)",
                                    (result[4], result[5], a1c),
                                )
                                conn.commit()
                            else:
                                print("Update Record")
                                ...
                                # with Lab Technician.
                                cur.execute(
                                    """UPDATE MachineResults SET 
                                                A1c = ?, 
                                                UpdateDate = ?, 
                                                Updated = ? 
                                            WHERE SampleID = ?""",
                                    (a1c, result[5], 1, result[4]),
                                )
                                conn.commit()
